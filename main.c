    ...
    for(int i=0;i<cfg.CmdCount; i++)
    {
        switch (cfg.Commands[i].cmd)
        {
            ...
            case ct_Hold:
                ...
                break;
            case ct_CenterZoom:
                CenterZoom(&state, cfg.Commands[i].cx, cfg.Commands[i].cy, cfg.Commands[i].zoom, cfg.Commands[i].steps);
                break;
            case ct_RotateZoom:
                RotateZoom(&state,cfg.Commands[i].angle, cfg.Commands[i].zoom, cfg.Commands[i].steps);
                break;
